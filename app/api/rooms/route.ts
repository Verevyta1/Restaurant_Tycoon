...
type RoomRow = { code:string; status:string; game_state:string|null; updated_at:number };
...
async function ensureSchema(){
  const db=env.DB;
  await db.batch([
    db.prepare("CREATE TABLE IF NOT EXISTS rooms (code TEXT PRIMARY KEY, status TEXT NOT NULL DEFAULT 'lobby', game_state TEXT, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL)"),
...
async function roomView(code:string){
  const room=await env.DB.prepare("SELECT code,status,game_state,updated_at FROM rooms WHERE code=?").bind(code).first<RoomRow>();
  if(!room)return null;
  ...
  return {code:room.code,status:room.status,gameState:room.game_state?JSON.parse(room.game_state):null,updatedAt:room.updated_at,players:players.results.map(p=>({id:p.id,name:p.name,seat:p.seat,isHost:Boolean(p.is_host)}))};
}
...
export async function POST(request:Request){
  try{
    ...
    if(action==="create"){
      ...
      await env.DB.batch([
        env.DB.prepare("INSERT INTO rooms(code,status,created_at,updated_at) VALUES(?,?,?,?)").bind(code,"lobby",now,now),
    ...
    if(action==="join"){
      ...
      if(room.players.length>=4)return json({error:"房间已经满员。"},409);
